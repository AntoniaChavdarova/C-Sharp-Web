        ...
        static async Task Main(string[] args)
        {
            ...
            try
            {
                ...
                //        ////samo do a - inner html link kum samata kushta
                     ...
                     //  var img = item.QuerySelector("#hotelsList >.result-item > .thumb > a > img")
                       //    .GetAttribute("content");
                       // item.
                       //Console.WriteLine(img);

                        //var img = item.QuerySelector("#hotelsList > div:nth-child(1) > div.thumb > a > img").GetAttribute("src");
                        //Console.WriteLine(img);
            ...
            foreach (var item in list)
            {
                ...
                Console.WriteLine(category.TextContent.TrimStart());
                ...
                Console.WriteLine(address.TextContent.TrimStart());
                ...
                Console.WriteLine(descriptionPage.TextContent.TrimStart());

                var udobstva = page.QuerySelectorAll("div.col-4 > div.extras > ul > li")
                    .Select(x => x.TextContent)
  
                .ToList(); ;
                Console.WriteLine($"Amenities:{string.Join(Environment.NewLine , udobstva)}");
              
                var tablewithInfo = page.QuerySelectorAll("#prices > table > tbody > tr > td").Select(x => x.TextContent.TrimStart())
                    .ToList();
               ...
               Console.WriteLine($" Cena lqto: {tablewithInfo[3]}");
                //  Console.WriteLine($" Cena zima: {winterPrice}");

                Console.WriteLine("Img");

                var img = page.QuerySelector(".gallery-slider > li > img").GetAttribute("src");
                Console.WriteLine(img);
                ;
